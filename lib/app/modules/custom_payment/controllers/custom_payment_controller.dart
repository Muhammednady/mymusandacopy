...
class CustomPaymentController extends GetxController {
  ...
  final Rx<ContractData> contractModel = ContractData().obs;
  ...
  // }
  validate(){
    return formOrderKey.currentState!.validate();
  }
  ...
  Future<void> getPayments({
    ...
    CustomPaymentProvider().payments(data).then(
      (response) {
        ...
        print('switch (paymentStatusFromRegExp(response.result.code)) {');
  ...
  Future<void> pay(amount, isFake) async {
    ...
    CustomPaymentProvider()
        ...
        .then((response) async {
          print('*************** checkout *******************');
          // print('response.result!.code! is ${response.result!.code!}');
          // print('brandType.value.name ${brandType.value.name}');
          // print('"checkoutID": ${response.id}');

      if (response.result!.code! == "000.200.100") {
        if (validate()) {
          try {
            print('  if (response.result!.code! == "000.200.100") {');
            final String result = await platform.invokeMethod(
              'get_hyperpay_response',
              {
                "type": "CustomUI",
            ...
            print(' ================log(result, name: "func => pay");=======================');
      ...
      print('catchError in Checkout process');
    });
    update();
  }
